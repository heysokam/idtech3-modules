...
/*
  ...
  It assumes that an i32 is at least 32 bits long
*/
...
static void copy64(u32* M, const byte* in) {
  ...
  for (i = 0; i < 16; i++) M[i] = ((u32)in[i * 4 + 3] << 24) | ((u32)in[i * 4 + 2] << 16) | ((u32)in[i * 4 + 1] << 8) | ((u32)in[i * 4 + 0] << 0);
}
...
static void mdfour_tail(const byte* in, i32 n) {
  ...
  Std_memset(buf, 0, 128);
  if (n) Std_memcpy(buf, in, n);
  buf[n] = 0x80;
